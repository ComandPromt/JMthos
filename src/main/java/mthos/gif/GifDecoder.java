...
public class GifDecoder {
	...
	protected int dispose; // last graphic control extension info

	protected int lastDispose; // 0=no action; 1=leave in place; 2=restore to bg; 3=restore to prev
	...
	protected int[] readColorTable(int ncolors) {
		...
		if (n < nbytes) {
			...
			while (i < ncolors) {
				...
				tab[i++] = 0xff000000 | ((((int) c[j++]) & 0xff) << 16) | ((((int) c[j++]) & 0xff) << 8)
						| (((int) c[j++]) & 0xff);
